...
def test_farmer_communication_keeps_timing_and_reported_vayego_separate(ffl_db, owner):
    ...
    first = board_for_source(ffl_db, evaluated_on=date(2026, 8, 8))
    ...
    assert second_vayego["delivery"]["state"] == "review_only"
...
def test_farmer_communication_rejects_unknown_cohort(ffl_db, owner):
    ...
    try:
        ...
        assert str(error) == "unknown farmer communication cohort"
